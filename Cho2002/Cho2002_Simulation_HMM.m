...
% All models have "dynamic belief" with a given a priori volatility.
...
MemTran = 0.013;            % memory decay for transitions
MemFreq = 0.019;            % memory decay for frequency
...
in.opt.pJ           = MemTran;          % prior probability of jump
...
in.jump             = 1;                % estimate with jumps
in.mode             = 'HMM';            % use the HMM (not sampling) algorithm
in.opt.pJ           = MemFreq;          % prior probability of jump
in.opt.ReturnDist   = 0;                % do not return full posterior distributions
...
save(sprintf('Simulation_HMM_Cho_%s.mat', timestamp), ...
    'Prediction_TransitionProbs', 'Prediction_StimProb', 'Prediction_AltProb', ...
    'L', 'N', 'MemTran', 'MemFreq', 'patterns_Cho')
