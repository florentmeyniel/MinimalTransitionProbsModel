...
% All observers have leaky integration and learn mostly from recent observations.
...
in.opt.MemParam     = {'Decay',   16};  % use a limited memory (exponential forgetting)
out                 = IdealObserver(in);
ObsLL{1}            = out.surprise;

% Run the Bernoulli observer (learn frequency of observation)
in.learned          = 'frequency';
in.opt.MemParam     = {'Decay',   10};  % use a limited memory (exponential forgetting)
out                 = IdealObserver(in);
